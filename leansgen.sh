...
# Device partitioning
sh -c "$(curl -sS -H 'Cache-Control: no-cache' https://raw.githubusercontent.com/LeandroTheDev/arch_linux/refs/heads/leansgen/Installation/disk_creation.sh)"
if [ $? -ne 0 ]; then
...
# Installing Linux
sh -c "$(curl -sS -H 'Cache-Control: no-cache' https://raw.githubusercontent.com/LeandroTheDev/arch_linux/refs/heads/leansgen/Installation/linux_install.sh)"
if [ $? -ne 0 ]; then
...
arch-chroot /mnt bash -c 'export INSTALLPARTITION="$INSTALLPARTITION" && sh -c "$(curl -sS -H 'Cache-Control: no-cache' https://raw.githubusercontent.com/LeandroTheDev/arch_linux/refs/heads/leansgen/Installation/device_config.sh)"'
if [ $? -ne 0 ]; then
